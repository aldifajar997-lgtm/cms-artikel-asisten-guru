...
posts.get('/', rateLimit(500, 60, 'public_posts'), async (c) => {
  ...
  let baseQuery = "SELECT p.id, p.slug, p.title, p.excerpt, p.featured_image, p.featured_image_alt, p.meta_title, p.meta_description, p.seo_score, p.reading_time_minutes, p.author_id, p.category_id, p.published_at FROM posts p"
  ...
  let finalQuery = `${baseQuery} WHERE ${whereClauses.join(' AND ')} ORDER BY p.published_at DESC LIMIT ?`
  params.push(limit)
  ...
  const nextCursor = formatted.length === limit ? formatted[formatted.length - 1].published_at : null

  return c.json({ data: formatted, limit, offset, next_cursor: nextCursor })
})
...
posts.get('/:slug', rateLimit(500, 60, 'public_posts'), async (c) => {
  ...
  const post = await c.env.DB.prepare(
    "SELECT id, slug, title, excerpt, content, featured_image, featured_image_alt, featured_image_caption, meta_title, meta_description, focus_keyword, secondary_keywords, seo_score, word_count, reading_time_minutes, author_id, category_id, status, published_at, created_at, updated_at FROM posts WHERE slug = ? AND status = 'published'"
  ).bind(slug).first()
...
const postSchema = z.object({
  title: z.string().trim().min(1).max(255).transform(cleanText),
  slug: z.string().trim().min(1).max(255).regex(slugRegex, 'Slug hanya boleh berisi huruf kecil, angka, dan tanda hubung').transform(cleanText),
  excerpt: z.string().max(500).optional().transform(v => v ? cleanText(v) : v),
  content: z.string().optional(),
  featured_image: z.string().optional(),
  featured_image_alt: z.string().optional().transform(v => v ? cleanText(v) : v),
  featured_image_caption: z.string().optional().transform(v => v ? cleanText(v) : v),
  meta_title: z.string().max(70).optional().transform(v => v ? cleanText(v) : v),
  meta_description: z.string().max(160).optional().transform(v => v ? cleanText(v) : v),
  focus_keyword: z.string().optional().transform(v => v ? cleanText(v) : v),
  secondary_keywords: z.array(z.string().transform(cleanText)).optional(),
  ...
  if (data.status === 'published' && (!data.content || data.content.trim().length < 50)) {
...
posts.post('/', requirePermission('create_post'), rateLimit(30, 60, 'post_write'), zValidator('json', postSchema), async (c) => {
  ...
  // Sanitasi HTML (Stored XSS Protection) dengan dukungan Iframe untuk Embed
  const cleanContent = body.content ? xss(body.content, { whiteList: { ...(xss as any).getDefaultWhiteList(), iframe: ['src', 'allow', 'allowfullscreen', 'frameborder', 'scrolling'] } }) : null
